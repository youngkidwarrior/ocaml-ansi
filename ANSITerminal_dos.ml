(* File: ANSITerminal_dos.ml
   Allow colors, cursor movements, erasing,... under DOS shells.
   *********************************************************************
...
(** See the file(s) ctlseqs.* (unix; in Debian package xspecs)
    (and http://www.ka.net/jmenees/Dos/Ansi.htm for DOS).

    CSI = "\027[" (ESC [)

    man console_codes
*)
(* FIXME: rather use the "console functions" of MS?
...
module Conio =
struct
  external clrscr : unit -> unit = "ANSITerminal_clrscr"
  external clreol : unit -> unit = "ANSITerminal_clreol"
  external gotoxy : int -> int -> unit = "ANSITerminal_gotoxy"
  external textcolor : int -> unit = "ANSITerminal_textcolor"
  external textbackground : int -> unit = "ANSITerminal_textbackground"
  external window : int -> int -> int -> int -> unit = "ANSITerminal_window"
  external highvideo : unit -> unit = "ANSITerminal_highvideo"
  external lowvideo : unit -> unit = "ANSITerminal_lowvideo"
  external normvideo : unit -> unit = "ANSITerminal_normvideo"
  external wherex : unit -> int = "ANSITerminal_wherex"
  external wherey : unit -> int = "ANSITerminal_wherey"

  type mode = BW40 | BW80 | C40 | C80 | C4350 | MONO
  external gettextinfo :
    unit -> int * int * int * int * mode * int * int * int  int
    = "ANSITerminal_gettextinfo"
end
...
type color =
    | Black | Red | Green | Yellow | Blue | Magenta | Cyan | White | Default
          (* Extra colors: *)
    | Lightgray | Darkgray | Lightred | Lightgreen | Lightblue
    | Lightmagenta | Lightcyan | Brown


(* Must be coherent with the order in conio.h *)
